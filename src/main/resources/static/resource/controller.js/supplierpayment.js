...
// -------------------------------------------------------------------------------------------------------------------
        ...
        //get total amount
                 ...
                 supplierpayment.paid_amount = totalPaidAmount;
                 textPaidAmount.value = parseFloat(totalPaidAmount).toFixed(2);

                 supplierpayment.balance_amount = totalBalanceAmount;
                 textBalanceAmount.value = parseFloat(totalBalanceAmount).toFixed(2);
 ...
 //need to set default color
    ...
    const refreshInnerGrnFormAndTable = () =>{
       ...
       grnpayment= {};
        ...
        // supplierpayment.SupplierPaymentHasGrnList.forEach(element =>{
        //    totalAmount = parseFloat(totalAmount) + parseFloat(element.line_cost);
        // })
...
const refillInnerGrnForm =(rowob , rowid) =>{
    ...
    const generateGrnTotalPrice=()=>{
        console.log("call generate total price")

        let selectedGRN = JSON.parse(selectGRN.value);

        let extIndex = supplierpayment.supplierPaymentHasGrnList.map(payment => payment.grn_id.id).indexOf(selectedGRN.id);
     
     console.log("Selected GRN Total Amount:", selectedGRN.grandtotal);
        ...
        if (extIndex != -1){
            ...
            textGrnTotalPrice.value = parseFloat(selectedGRN.grandtotal).toFixed(2);
        ...
        textGrnBalance.value  = "";
        grnpayment.line_cost = null ;
        textGrnBalance.style.border = "1px solid #ced4da";

      


        btnAddSupPayment.disabled = false;
    ...
    const calculateBalance = () =>{
       let qyt = textGrnPaidPrice.value;

       if(new RegExp("^[1-9][0-9]{0,7}[.][0-9]{2}$").test(qyt)){
        textGrnBalance.value =(parseFloat(textGrnTotalPrice.value) - parseFloat(qyt)).toFixed(2);

        grnpayment.balance_amount = textGrnBalance.value;
