...
const reFreshProductForm = () => {
    ...
    fillDataIntoSelect(selectStatus, 'Select Status *', productStatusList, 'name');
   ...
   fillDataIntoSelect(selectSize, 'Select Size*', sizeList, 'name');
    ...
    imgProductphoto.src = "/resource/Images/No_Image_Available.jpg";
...
const refillProductForm =(rowOb,rowInd)=>{
    ...
    textROQ.value = product.reorder_quantity;
...
const checkError = () => {
    ...
    if (product.reorder_quantity == null){
        errors = errors + 'please Enter Product Reorder Quantity...! \n';
    }
...
const checkUpdate = ()=>{
    ...
    if (product.reorder_quantity != oldProduct.reorder_quantity){
        updates = updates + " Product Reorder Quantity is change " + oldProduct.reorder_quantity + " into " + product.reorder_quantity +"\n";
    }
...
const buttonClearImage =() =>{
    if(product.image != null){
        ...
        product.image = null;
        imgUserphoto.src = "/resource/Images/No_Image_Available.jpg";
        textProductphoto.value="";
...
const refreshInnerMaterialFormAndTable = () =>{
    ...
    // if (totalAmount == 0.00){
    //     textUnitPrice.value = 'Total Material Cost';
    //     textUnitPrice.style.border = '1px solid #ced4da'
    //     textUnitPrice.disabled = true;
    //     promaterial.unit_price = null;
    // }
    // else{
    //     textUnitPrice.disabled = false;
    //     textUnitPrice.value = parseFloat(totalAmount).toFixed(2);
    //     textUnitPrice.style.border = '2px solid green'
    //     promaterial.unit_price = productMaterialCost.value;
    // }
