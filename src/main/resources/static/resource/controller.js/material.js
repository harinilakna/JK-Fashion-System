...
const refreshMaterialTable = () =>{
 ...
 const displayProperty = [{property:'code', datatype:'string'} ,
                        ...
                        //  {property:'photo', datatype:'string'},
        ...
        materials.forEach((element, index) => {
            if(element.material_status_id.name == 'Out of stock'){
                    ...
                    tableMaterial.children[1].children[index].children[8].children[1].disabled = true; //you can also use disabled
...
const filterSubCategory = () =>{
    ...
    const refreshMaterialForm = () => {
        ...
        selectStatus.style.border = "2px solid green";
            ...
            textAvailableQuatity.value ="";
            textUnitPrice.value ="";
            formFile.value ="";

            textRop.style.border = '1px solid #ced4da';
            textROQ.style.border = '1px solid #ced4da';
            textTotalQuantity.style.border = '1px solid #ced4da';
            textAvailableQuatity.style.border = '1px solid #ced4da';
            textUnitPrice.style.border = '1px solid #ced4da';
            formFile.style.border = '1px solid #ced4da';
 ...
 const generateItemName = ()=>{
    ...
    const refillMaterialForm =(rowOb,rowInd)=>{
      ...
      //get data list for select element
        ...
        if(material.total_quantity != null)
            textTotalQuantity.value = material.total_quantity;
            else textTotalQuantity.value = "";

        if(material.unit_price != null)
            textUnitPrice.value = material.unit_price;
                else textUnitPrice.value = "";

        if(material.available_quantity != null)
            textAvailableQuatity.value = material.available_quantity;
                    else textAvailableQuatity.value = "";
    

        if(material.photo != null)
        formFile.value = material.photo;
        else formFile.value = ""; 
...
const checkUpdate = ()=>{
    ...
    if(material.photo != oldmaterial.photo){
        updates = updates + "Photo is change \n";
    }
    ...
    if(material.total_quantity != oldmaterial.total_quantity){
        updates = updates + "total quantity is change \n";
    }

    if(material.available_quantity != oldmaterial.available_quantity){
        updates = updates + "Available quantity is change \n";
    }


    if(material.reorder_quantity != oldmaterial.reorder_quantity){
        updates =  updates + "Re-Order Quantity is change \n";
    }
