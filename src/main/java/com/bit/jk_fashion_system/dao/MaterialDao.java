...
public interface MaterialDao extends JpaRepository<Material, Integer> {
    ...
    public List<Material>getAvailableMaterialList();
     ...
     //define query for get active items with selected coloumn
     @Query(value = "SELECT new Material(M.id, M.code, M.name) From Material M where M.material_status_id.id=1 and M.id not in (select shi.material_id.id from SupplierHasMaterial shi where shi.supplier_id.id=?1)")
